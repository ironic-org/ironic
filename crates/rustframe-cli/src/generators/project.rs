...
pub fn create(
    ...
    let files = [
        ...
        (
            destination.join("rustframe.toml"),
...
fn manifest(name: &str, workspace: Option<&Path>) -> String {
    let dependencies = workspace.map_or_else(
        || {
            format!(
                "rustframe = \"{0}\"\nrustframe-platform-axum = \"{0}\"",
                env!("CARGO_PKG_VERSION")
            )
        },
        |workspace| {
            format!(
                "rustframe = {{ path = \"{}\" }}\nrustframe-platform-axum = {{ path = \"{}\" }}",
                toml_path(&workspace.join("crates/rustframe")),
                toml_path(&workspace.join("crates/rustframe-platform-axum"))
            )
        },
    );
    format!(
