...
### Requirement: HttpError SHALL support optional backtrace capture
The framework SHALL capture a backtrace when `HttpError::internal()` is called and the `backtrace` feature is enabled.
...
The `ironic debug` command SHALL start an interactive REPL that allows inspecting the DI container, module graph, and loaded configuration at runtime.
...
The framework SHALL provide helper utilities for creating test data, including a `FixtureBuilder` for constructing entities with sensible defaults.
