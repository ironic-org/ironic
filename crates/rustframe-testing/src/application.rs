    ...
    fn drop(&mut self) {
        ...
        let cleanup = std::thread::spawn(move || {
            tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .expect("RustFrame test cleanup runtime must initialize")
        ...
        match cleanup {
            Ok(Ok(())) => {}
            Ok(Err(error)) => panic!("RustFrame test application cleanup failed: {error}"),
            Err(payload) => {
                let message = payload
                    .downcast_ref::<&str>()
                    .copied()
                ...
                panic!("RustFrame test application cleanup panicked: {message}");
            }
        }
    }
}
