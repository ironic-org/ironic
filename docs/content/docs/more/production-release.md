...
## Framework vs application releases

Ironic uses a two-tier release model:

| Tier | What | Tools |
|------|------|-------|
| **Framework** | `ironic` crate & sub-crates on crates.io | `scripts/release.sh` + GitHub Actions |
...
- [x] Framework releases are fully automated via `scripts/release.sh` + GitHub Actions
