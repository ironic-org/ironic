...
pub fn generate_app(root: &Path, name: &str) -> Result<GenerationReport, CliError> {
    ...
    let app_toml = crate::generators::project::app_manifest(&names.kebab);

    let mut files: Vec<(std::path::PathBuf, String)> = Vec::new();
    files.push((dest.join("Cargo.toml"), app_toml));
    files.push((dest.join("src/main.rs"), app_main(&names)));
    files.push((dest.join("src/app.rs"), app_module(&names)));
    files.push((dest.join("src/modules/mod.rs"), "pub mod health;\n".into()));

    // Health module
    files.push((dest.join("src/modules/health/mod.rs"),
        r#"use ironic::prelude::*;
...
"#.into()));
    files.push((dest.join("src/modules/health/controller/mod.rs"),
        "pub mod health_controller;\n".into()));
    files.push((dest.join("src/modules/health/controller/health_controller.rs"),
        r#"use ironic::prelude::*;
