...
impl AdminController {
    #[get("/dashboard")]
    #[guard(RoleGuard::new(&["admin"]))]
    async fn dashboard(&self) -> Result<Json<Dashboard>, HttpError> {
        // ...
    }
}
```

### Attribute macro — applied with `#[guard]`
...
#[controller("/auth")]
#[derive(Injectable)]
#[guard(ApiKeyGuard::new(&["sk-master"]))]   // ← applies to every route
pub struct AuthController { }

#[routes]
impl AuthController {
    #[get("/me")]
    #[guard(AuthGuard)]                        // ← applies only to this route
...
> `#[guard(...)]` accepts any expression that evaluates to a type implementing `Guard`. Multiple `#[guard]` attributes on the same item add guards in the order they appear.
...
| `#[guard]` on a struct without `Guard` trait | Make sure the type passed to `#[guard(...)]` implements `Guard` |
...
- [x] Register guards globally, per-controller, per-route, or with `#[guard]`
