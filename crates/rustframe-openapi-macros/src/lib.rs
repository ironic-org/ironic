...
fn expand(input: DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    ...
    for field in fields.named {
        ...
        properties.push(quote! {
            properties.insert(
                #field_name.to_owned(),
                <#ty as ::rustframe_openapi::OpenApiSchema>::openapi_schema(),
            );
        });
        if !is_option(&ty) {
    ...
    Ok(quote! {
        impl ::rustframe_openapi::OpenApiSchema for #name {
            fn openapi_schema() -> ::rustframe_openapi::__private::serde_json::Value {
                let mut properties = ::rustframe_openapi::__private::serde_json::Map::new();
                #(#properties)*
                ::rustframe_openapi::__private::serde_json::json!({
                    "type": "object",
