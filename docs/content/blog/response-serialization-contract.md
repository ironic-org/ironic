...
# FrameworkResponse and IntoFrameworkResponse — the protocol-neutral boundary
...
- `FrameworkResponse` itself — passthrough identity.
- `()` — `204 No Content`.
...
The `Result` impl is the one that matters most in practice. A handler returns `Result<Json<User>, HttpError>`. The `Ok` arm takes the JSON path: `Serialize` into bytes, set the content-type header, wrap in `FrameworkResponse`. The `Err` arm hits `HttpError`'s own implementation.

---

## Structured errors: a contract across every adapter

When `HttpError` converts to a response (`error.rs:87`), it calls `FrameworkResponse::error()`:
...
fn error_response(error: HttpError) -> Response {
    match error.into_framework_response() {
        ...
        Err(_) => framework_response(FrameworkResponse::empty(INTERNAL_SERVER_ERROR)),
    }
}
```

The adapter never inspects the error code. It never reads the JSON body. It never knows what `RF_HTTP_GUARD_DENIED` means. It just calls `into_framework_response()` on the `HttpError`, gets back a `FrameworkResponse`, and mechanically translates headers and bytes. If tomorrow Ironic adds fifty new error codes, the Axum adapter is unchanged. If someone writes an Actix adapter next week, they write the same two mechanical functions.
...
Handler result
    │
    ▼
IntoFrameworkResponse::into_framework_response()
    │
    ▼
FrameworkResponse (status + headers + bytes)
    │
    ▼
...
The framework never produces platform-specific types. The adapter never knows framework semantics. They only meet at `FrameworkResponse` — three fields, fully materialized, no secrets.
